    ...
    try {
        ...
        switch($requestMethod) {
            case 'PUT':
                ...
                $clientInfo = $client->updateClient();

                if(!empty($clientInfo)) {
                    $js_encode = json_encode(array('status'=>'SUCCESS', 'message'=>'Client updated Successfully.'), true);
                } else {
                    $js_encode = json_encode(array('status'=>'FAILED', 'message'=>'Client update failed.'), true);
                }
                echo $js_encode;    
                break;
